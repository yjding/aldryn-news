...
from aldryn_news.utils import FormatableReverse
...
urlpatterns = patterns('',
    ...
    url(r'^(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/$', redirect_to, {'url': FormatableReverse('archive-month', keys=['year', 'month'])}),
    ...
    url(r'^(?P<category_slug>[-\w]+)/(?P<year>\d{4})/$', redirect_to, {'url': FormatableReverse('archive-year', keys=['year'])}),
    url(r'^(?P<category_slug>[-\w]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/$', redirect_to, {'url': FormatableReverse('archive-month', keys=['year', 'month'])}),
    url(r'^(?P<category_slug>[-\w]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/$', redirect_to, {'url': FormatableReverse('archive-month', keys=['year', 'month'])}),
